...
from family.forms import FamilyMemberForm, HobbyForm
from django.db.models import Count
from django.shortcuts import get_object_or_404

def update_family(request, pk):
    head = get_object_or_404(FamilyHead, pk=pk)
...
def family_list(request):
    heads = FamilyHead.objects.annotate(member_count=Count('members'))
    members = FamilyMember.objects.all()
...
def view_family(request, pk):
    ...
    members = FamilyMember.objects.filter(family_head_id=pk).all().exclude(status=statusChoice.DELETE)
    hobbies = Hobby.objects.filter(family_head_id=pk).all().exclude(status=statusChoice.DELETE)
...
def update_single_member(request, pk):
    member = FamilyMember.objects.get(id=pk)  # fetch single member
    ...
    return render(request, "update_single_member.html", context)
